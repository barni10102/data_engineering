"""Analytics API router exposing summary, sector stats, and backtest endpoints."""
...
# Analytics endpoints power aggregated dashboard panels and lightweight DS views.
...
def read_assets_summary(
    ...
    # Keep from/to aliases for Grafana compatibility with standard time parameter names.
    date_from: Optional[datetime] = Query(None, alias="from"),
    date_to: Optional[datetime] = Query(None, alias="to"),
    limit: int = Query(30, ge=1, le=500),
):
    """Return aggregated asset summary rows for the selected time window."""
...
def read_sp500_sector_stats(
    ...
    """Return current sector-level S&P500 aggregates from reference tables."""
...
def read_forecast_backtest(
    ...
    """Return time-series backtest payload (EDA, split, metrics, forecast points)."""
